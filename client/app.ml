...
let view_input_content ~inject content =
  view_text_input ~inject
    ~extra_attrs:Virtual_dom.Vdom.Attr.[ autofocus true
                                       ; id "content" ]
    "content" content
...
let view_input_name ~inject name =
  view_text_input ~inject
    "name" name
    (fun _ value -> inject (Action.Input_name value))
;;
...
let view model ~inject =
  ...
  and     messages = model >>| Model.messages >>| Fqueue.to_list
  in
  let open Virtual_dom.Vdom in
  Node.div []
